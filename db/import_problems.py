...
def main():
    ...
    source_rows = [{"slug": slug, "title": title} for slug, title in sources.items()]
    quiz_rows = [{"slug": slug, "title": title, "source": src}
                 for slug, (title, src) in quizzes.items()]
    problem_rows = [
        {"slug": p["slug"], "title": p["title"], "source": quizzes[p["quiz_slug"]][1],
         "difficulty": None, "tags": [], "hero": HERO, "dealer": DEALER,
         "vulnerability": VULN, "deal": p["deal"], "auction": p["auction"],
         "play": None, "contract": None, "commentary": None}
        for p in problems
    ]
    link_rows = [{"quiz_slug": q, "problem_slug": ps, "ordinal": o} for q, ps, o in links]
    print(emit_sql(source_rows, quiz_rows, problem_rows, link_rows), end="")
